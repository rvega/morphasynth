...
void PdUIMoogLopass::addToggleButton(const char* label, float* zone)
...
void PdUIMoogLopass::addNumDisplay(const char* label, float* zone, int precision) {}
void PdUIMoogLopass::addTextDisplay(const char* label, float* zone, const char* names[], float min, float max) {}
...
void PdUIMoogLopass::openFrameBox(const char* label)
{
  if (!path.empty()) path += "/";
  path += mangle(label);
}
  ...
  public:
	...
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			float 	fRec3 = fRec7[0];
			output0[i] = (FAUSTFLOAT)(((((fTemp31 * fRec2[1]) + ((fTemp7 * fTemp30) / fTemp17)) + ((fRec2[0] * fTemp14) / fTemp9)) + ((fRec3 * ((1.0f - fTemp6) - (fTemp12 * fTemp14))) / (sqrtf(fTemp13) * fTemp9))) / fTemp5);
			// post processing
			fRec2[1] = fRec2[0];
			fRec7[1] = fRec7[0];
			fRec4[1] = fRec4[0];
