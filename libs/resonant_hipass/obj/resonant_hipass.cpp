...
class PdUIResonantHipass : public UI
{
public:
  int nelems;
  ui_elem_t *elems;
  ...
  PdUIResonantHipass();
  PdUIResonantHipass(const char *s);
  virtual ~PdUIResonantHipass();
...
PdUIResonantHipass::PdUIResonantHipass()
{
  nelems = 0;
  elems = NULL;
  path = "";
...
PdUIResonantHipass::PdUIResonantHipass(const char *s)
{
  nelems = 0;
  elems = NULL;
  path = s?s:"";
...
inline void PdUIResonantHipass::add_elem(ui_elem_type_t type, const char *label)
{
...
inline void PdUIResonantHipass::add_elem(ui_elem_type_t type, const char *label, float *zone)
{
  ui_elem_t *elems1 = (ui_elem_t*)realloc(elems, (nelems+1)*sizeof(ui_elem_t));
  if (elems1)
    elems = elems1;
...
void PdUIResonantHipass::addButton(const char* label, float* zone)
{ add_elem(UI_BUTTON, label, zone); }
void PdUIResonantHipass::addToggleButton(const char* label, float* zone)
{ add_elem(UI_TOGGLE_BUTTON, label, zone); }
void PdUIResonantHipass::addCheckButton(const char* label, float* zone)
{ add_elem(UI_CHECK_BUTTON, label, zone); }
void PdUIResonantHipass::addVerticalSlider(const char* label, float* zone, float init, float min, float max, float step)
{ add_elem(UI_V_SLIDER, label, zone, init, min, max, step); }
void PdUIResonantHipass::addHorizontalSlider(const char* label, float* zone, float init, float min, float max, float step)
{ add_elem(UI_H_SLIDER, label, zone, init, min, max, step); }
void PdUIResonantHipass::addNumEntry(const char* label, float* zone, float init, float min, float max, float step)
{ add_elem(UI_NUM_ENTRY, label, zone, init, min, max, step); }

// FIXME: addNumDisplay and addTextDisplay not implemented in Faust yet?
void PdUIResonantHipass::addNumDisplay(const char* label, float* zone, int precision) {}
void PdUIResonantHipass::addTextDisplay(const char* label, float* zone, const char* names[], float min, float max) {}
void PdUIResonantHipass::addHorizontalBargraph(const char* label, float* zone, float min, float max)
{ add_elem(UI_H_BARGRAPH, label, zone, min, max); }
void PdUIResonantHipass::addVerticalBargraph(const char* label, float* zone, float min, float max)
{ add_elem(UI_V_BARGRAPH, label, zone, min, max); }

void PdUIResonantHipass::openFrameBox(const char* label)
{
  if (!path.empty()) path += "/";
  path += mangle(label);
}
void PdUIResonantHipass::openTabBox(const char* label)
{
  if (!path.empty()) path += "/";
  path += mangle(label);
}
void PdUIResonantHipass::openHorizontalBox(const char* label)
{
  if (!path.empty()) path += "/";
  path += mangle(label);
}
void PdUIResonantHipass::openVerticalBox(const char* label)
{
  if (!path.empty()) path += "/";
  path += mangle(label);
}
void PdUIResonantHipass::closeBox()
{
  int pos = path.rfind("/");
  if (pos < 0) pos = 0;
  path.erase(pos);
...
class mydsp : public dsp {
  ...
  public:
	...
	virtual void instanceInit(int samplingFreq) {
		...
		fConst0 = (6.283185307179586f / min(192000, max(1, fSamplingFreq)));
	...
	virtual void buildUserInterface(UI* interface) {
		interface->openVerticalBox("resonant_hipass");
		interface->addHorizontalSlider("frequency", &fslider1, 1e+03f, 1.0f, 2.2e+04f, 1.0f);
		interface->addHorizontalSlider("q", &fslider0, 1.0f, 0.07f, 1e+02f, 0.01f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp0 = (fConst0 * max(0, fRec2[0]));
...
static void *faust_new(t_symbol *s, int argc, t_atom *argv)
{
  ...
  x->dsp = new mydsp();
  x->ui = new PdUIResonantHipass(id?id->s_name:NULL);
