#define mydsp resonant_hipass
//-----------------------------------------------------
// name: "resonant_hipass"
//
...
class mydsp : public dsp {
  ...
  public:
	static void metadata(Meta* m) 	{ 
		m->declare("name", "resonant_hipass");
	...
	virtual void buildUserInterface(UI* interface) {
		...
		interface->addHorizontalSlider("frequency", &fslider0, 1e+03f, 1.0f, 2.2e+04f, 1.0f);
		interface->addHorizontalSlider("q", &fslider1, 1.0f, 0.07f, 1e+02f, 0.01f);
		interface->closeBox();
	}
	virtual void compute (int count, FAUSTFLOAT** input, FAUSTFLOAT** output) {
		...
		for (int i=0; i<count; i++) {
			...
			float fTemp4 = (0.5f * (fTemp3 * fRec0[1]));
			float fTemp5 = (1 + fTemp1);
			fRec0[0] = (((1 - fTemp1) - (((0 - (2 * fTemp2)) * fRec0[2]) + fTemp4)) / fTemp5);
			output0[i] = (FAUSTFLOAT)(((float)input0[i] * fRec0[0]) + ((fTemp4 + (fRec0[2] * (0 - fTemp3))) / fTemp5));
			// post processing
...
extern "C" void resonant_hipass_setup(mydsp)
{
