...
Morphasynth.TimbreSpace = function(){
  ...
  this.init = function(){
    ...
    //the pressed
    $('#timbre-space').on('mousedown touchstart', $.proxy(this.pressedKey, this));

    //the released
    $('#timbre-space').on('mouseup touchend', $.proxy(this.releasedKey, this));

    $("#timbre-space").on('mousemove touchmove', function(e) {
      if(self.sendXY){
        self.pointerX = e.originalEvent.touches[0].pageX;
        self.pointerY = e.originalEvent.touches[0].pageY;
  ...
  this.pressedKey = function(e){
    // if(typeof event.clientY==='undefined' || typeof event.clientX==='undefined'){
      // return;
    // }

    var px = e.originalEvent.touches[0].pageX;
    var py = e.originalEvent.touches[0].pageY;
    this.sendPoses(px, py);
    this.sendXY=true;
  };
