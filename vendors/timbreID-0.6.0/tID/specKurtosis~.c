...
static void specKurtosis_tilde_bang(t_specKurtosis_tilde *x)
{
    ...
    t_float dividend, centroid, std, divisor, kurtosis, *windowFuncPtr;
	double currentTime;
	...
	for(i=0; i<=windowHalf; i++)
		dividend += pow((x->binFreqs[i] - centroid), 4) * signal_R[i];
...
static void specKurtosis_tilde_powerSpectrum(t_specKurtosis_tilde *x, t_floatarg spec)
{
    spec = (spec<0)?0:spec;
    spec = (spec>1)?1:spec;
...
static void *specKurtosis_tilde_new(t_symbol *s, int argc, t_atom *argv)
{
    ...
    post("specKurtosis~: window size: %i", (int)x->window);
...
// compare n to stored n and recalculate filterbank if different
	if(sp[0]->s_sr != (x->sr*x->overlap) || sp[0]->s_n != x->n)
	{
    	...
    	post("specKurtosis~: window size: %i. overlap: %i. sampling rate: %i, block size: %i", (int)x->window, x->overlap, (int)x->sr, (int)x->n);
	};
};
