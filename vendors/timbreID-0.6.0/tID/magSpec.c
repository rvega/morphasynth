...
version 0.0.5, December 21, 2011

¥ 0.0.5 incorporates the tIDLib header functions.
...
static void magSpec_bang(t_magSpec *x)
{
	...
	if(lengthSamp > x->powersOfTwo[x->powTwoArrSize-1])
	{
		post("WARNING: magSpec: window truncated because requested size is larger than the current max_window setting. Use the max_window method to allow larger windows. Sizes of more than 131072 may produce unreliable results.");
		lengthSamp = x->powersOfTwo[x->powTwoArrSize-1];
		window = lengthSamp;
	}

	magSpec_analyze(x, startSamp, lengthSamp);
...
static void magSpec_powerSpectrum(t_magSpec *x, t_floatarg spec)
{
	...
	if(x->powerSpectrum)
		post("using power spectrum for magSpec computation.");
	else
		post("using magnitude spectrum for magSpec computation.");
}
...
static void *magSpec_new(t_symbol *s)
{
	...
	x->powerSpectrum = 0; // choose mag (0) or power (1) spec in the magSpec computation
