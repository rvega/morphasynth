...
mfcc~ - A Mel Frequency Cepstrum Analysis external.
...
version 0.2.9, December 20, 2011

¥ 0.2.9 incorporating tIDLib.h.  re-jiggering the way samplerate/overlap works in _dsp
...
static void mfcc_tilde_bang(t_mfcc_tilde *x)
{
	...
	tIDLib_cosineTransform(x->mfcc, signal_R, x->numFilters);
...
static void mfcc_tilde_hat(t_mfcc_tilde *x, t_floatarg filt)
{
	int i, idx;
...
static void mfcc_tilde_window(t_mfcc_tilde *x, t_floatarg ws)
{
...
// compare n to stored n and recalculate filterbank if different
	if( sp[0]->s_sr != (x->sr*x->overlap) || sp[0]->s_n != x->n )
	{
    	...
    	post("mfcc~: window size: %i. overlap: %i. sampling rate: %i, block size: %i", (int)x->window, x->overlap, (int)x->sr, (int)x->n);
	};
